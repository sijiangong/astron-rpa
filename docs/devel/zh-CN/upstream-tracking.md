...
| 上游提交 | 标题 | 结论 | 原因 |
|---|---|---|---|
| `be240e5f` | Update router binary with LAN access guard (#811) | **暂缓，不吸收** | ①仓库里没有对应的 Go 源码（`route.exe` 仅 win 版），是**不可审计的上游二进制**，不宜写进自家产品的发行物；②功能与已移植的 `9b654dc9`（Python 侧 guard，见第 2 节）重叠；③只有 Windows 版，未来做 Linux/mac 客户端也没有对应物。若日后需要，应自行实现或等上游开源其 Go 源码 |
